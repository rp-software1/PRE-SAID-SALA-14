...
const estadoConfig: Record<string, { label: string; color: string; bg: string; border: string; icon: string }> = {
  disponible: { label: "Disponible", color: "#10b981", bg: "rgba(16,185,129,0.1)",  border: "rgba(16,185,129,0.3)",  icon: "✓" },
  ocupada:    { label: "Ocupada",    color: "#ef4444", bg: "rgba(239,68,68,0.1)",   border: "rgba(239,68,68,0.3)",   icon: "●" },
  reservada:  { label: "Reservada",  color: "#eab308", bg: "rgba(234,179,8,0.1)",   border: "rgba(234,179,8,0.3)",   icon: "★" },
};
...
export default function MesasPage() {
  ...
  const fetchMesas = async () => {
    try {
      ...
      setError(true);
      notify("Error al cargar mesas", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const crearMesa = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNumero(""); setCapacidad(""); setShowForm(false);
      fetchMesas();
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "2rem", flexWrap: "wrap", gap: "1rem" }}>
        <div>
          <p style={{ margin: "0 0 4px", fontSize: "0.78rem", fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--green)" }}>
            Plano del local
          </p>
          ...
          {Object.entries(estadoConfig).map(([key, cfg]) => {
            ...
            return (
              <div key={key} style={{
                ...
                fontSize: "0.82rem", fontWeight: 600, color: cfg.color,
              }}>
                <span>{cfg.icon}</span>
          ...
          <form onSubmit={crearMesa}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto", gap: "12px", alignItems: "end" }}>
              <div>
                ...
                <input type="number" placeholder="Ej: 5" value={numero}
                  onChange={(e) => setNumero(e.target.value)} required min="1" style={inputStyle} />
              </div>
              <div>
                <label style={{ display: "block", fontSize: "0.78rem", fontWeight: 600, color: "var(--text-secondary)", marginBottom: "6px" }}>
                  Capacidad (personas)
                </label>
                <input type="number" placeholder="Ej: 4" value={capacidad}
                  onChange={(e) => setCapacidad(e.target.value)} required min="1" style={inputStyle} />
              </div>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                    {(["disponible", "ocupada", "reservada"] as const)
                      .filter(s => s !== mesa.estado)
                      .map(s => {
                        const sc = estadoConfig[s];
                        return (
                          <button
                            key={s}
                            onClick={() => setStateChange({ id: mesa.id, newState: s })}
                            style={{
                              display: "flex", alignItems: "center", justifyContent: "center", gap: "6px",
                              padding: "8px", borderRadius: "8px",
                              background: sc.bg, border: `1px solid ${sc.border}`,
                              color: sc.color, fontSize: "0.82rem", fontWeight: 600, cursor: "pointer",
                            }}
                          >
